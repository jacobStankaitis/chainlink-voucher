...
export const RequestProvider = ({ children }) => {
  ...
  const addVoucher = async (voucher) => {
    ...
    const gas = await contract.estimateGas.addVoucher(voucher.id, voucher.hash, voucher.expire_date, voucher.email, {
      value: amountWei._hex
    });
    let tx = await contract.addVoucher(voucher.id, voucher.hash, voucher.expire_date,voucher.email, {
      value: amountWei._hex,
  ...
  const addTokenVoucher = async (voucher) => {
    ...
    const gas = await contract.estimateGas.addTokenVoucher(voucher.id, voucher.hash, voucher.expire_date, USDC_CONTRACT, amountWei._hex,voucher.email, {
      value: amountWei._hex
    });
    let tx = await contract.addTokenVoucher(voucher.id, voucher.hash, voucher.expire_date, USDC_CONTRACT, amountWei._hex,voucher.email, {
      gasLimit: parseInt(gas * 1.5)
    });
    return tx.wait();
